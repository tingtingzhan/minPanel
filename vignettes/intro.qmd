...
1. The `S3` method dispatch `subset.panel()` (Appendix @sec-positive, @sec-subset) selects the collections that identify at least one positive subjects *per collection*.  
2. The `S3` method dispatch `subset.panel()` (Appendix @sec-positive, @sec-subset) selects the collections that identify none of the negative subjects *per collection*.  The criterion *`'False(+) ≤0/31'`* is `append`ed to the `@label` of the output.
3. The `S3` method dispatch `sort_by.panel()` (Appendix @sec-positive, @sec-sortby) sorts the collections by their number of true positives.
4. The `S3` method dispatch `subset.panel()` (Appendix @sec-cum-positive, @sec-subset) identifies the collections, so that each one collection identifies at least one more positive subjects than all the preceding collections.
5. Function `append_label()` (Appendix @sec-cum-positive, @sec-appendlabel) summarizes the number of negative subjects identified *as a panel of collections*.  The criterion *`'cumFalse(+) ≤0/31'`* is `append`ed to the `@label` of the output
...
| [`autoplot`](https://search.r-project.org/CRAN/refmans/ggplot2/html/autoplot.html), [`autolayer`](https://search.r-project.org/CRAN/refmans/ggplot2/html/autolayer.html) | Create a ggplot (layer), from package **`ggplot2`** [@ggplot2] |
| [`consort_plot`](https://search.r-project.org/CRAN/refmans/consort/html/consort_plot.html) | Consolidated Standards of Reporting Trials (CONSORT) diagram, from package **`consort`** [@consort] |
| [`flextable`](https://search.r-project.org/CRAN/refmans/flextable/html/flextable.html), [`as_flextable`](https://search.r-project.org/CRAN/refmans/flextable/html/as_flextable.html) | (Transform an R object into) flexible table |
...
| [`theme`](https://search.r-project.org/CRAN/refmans/ggplot2/html/theme.html) | Modify components of a theme for ggplot, from package **`ggplot2`** [@ggplot2] |
