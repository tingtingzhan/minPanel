...
# Selected `panel` {#sec-select-panel}
...
5. Function `append_label()` (Appendix @sec-appendlabel) summarizes the number of negative subjects identified *as a panel of collections* (Appendix @sec-cum-positive).  The [*panel-wise* criterion]{style="background-color: #FFFF00"} *`'panelFalse(+) ≤0/31'`* is `append`ed to the `@label` of the output.
...
User may impose an additional criteria of panel-wise false positive of 5 or less out of 31 negative subjects.
